...
public class Main {
	...
	public static void main(String args[]){
		printInstructions();
		String request = "";
		while(request.equals("y") || request.equals("")) {
			request = "";
			int[] amounts = getAmounts();
			playBlackJack(amounts[0], amounts[1]);
			System.out.println("Game over! You played " + game.rounds + " rounds. You won " + game.winningRounds + " rounds and lost " + game.losingRounds +" rounds.");
			System.out.println("Of all games you have played this session: Total money betted: " + game.betTotal + " Average bet: " + game.betAvg);
			while(!(request.equals("y")) && !(request.equals("n"))) {
				System.out.println("Play again? (y/n)");
				request = reader.nextLine();
				if(!(request.equals("n")) && !(request.equals("y")))
				{
